...
const BreadCrum: React.FC<BreadCrumProps> = ({ onSearch, setSelectedFilter, selectedFilter, activeIndex }) => {

    const [isOpen, setIsOpen] = useState(false);
    const dropdownRef = useRef<HTMLDivElement>(null);
    // const options = ['Delivered', 'Processing', 'Shipped', 'Cancelled'];
    const allOptions: Record<number, string[]> = {
        1: ['Delivered', 'Processing', 'Shipped', 'Cancelled'],
        3: ['Healthy', 'Low', 'Critical', 'Out of Stock'],
    };

    const options = allOptions[activeIndex] ?? [];
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <div className="flex items-center px-3 gap-2 border border-[#D5D5D5] rounded-full w-[388px] h-[38px] focus-within:ring-1 focus-within:ring-[#11401C] focus-within:border-[#11401C] transition-all">
                        ...
                        <input
                            type="text"
                            placeholder={placeholders[activeIndex] ?? "Search"}
