...
import { sessionVideoSchema } from "@/lib/validation/sessionVideoSchema";
import { SessionVideo } from "@/lib/api/sessionVideoApi";

type ModalProps = {
    isOpen: boolean;
    onClose: () => void;
    editingVideo: SessionVideo | null;
    onSubmit: (values: { title: string; description: string; symptoms: string[]; videos: File[] }, isDraft: boolean) => Promise<void>;
    isLoading?: boolean;
};
...
const Modal: React.FC<ModalProps> = ({ 
    ...
    const initialValues = {
        ...
        symptoms: editingVideo?.symptoms || [] as string[],
        videos: [] as File[],
    };
        ...
        <div className="fixed inset-0 bg-[#BABBBB]/40 bg-opacity-50 flex items-center justify-center z-30">
            <div className="bg-white rounded-lg px-4 py-3 w-full max-w-2xl relative max-h-[42.7rem] overflow-y-auto scrollbar-hide">
                ...
                <Formik
                    ...
                    validationSchema={sessionVideoSchema}
                ...
                >
                    {({ values, setFieldValue, errors, touched, setErrors, setFieldTouched, isSubmitting }) => {
                        ...
                        return (
                            <Form>
                                <div>
                                    {SessionVideosModalData.map((section, sectionIndex) => (
                                        <div key={sectionIndex}>
                                            ...
                                            <div className="grid grid-cols-1 gap-2 mt-2">
                                                {section.fields.map((field, index) => {
                                                    ...
                                                    const isSymptoms = field.label === "Symptoms Tags";
                                                    return (
                                                        <div key={index}>
                                                                        ...
                                                                        {values.symptoms?.map((symptom, idx) => (
                                                                            ...
                                                                            >
                                                                                ...
                                                                                <button
                                                                                    type="button"
                                                                                    onClick={() => {
                                                                                        const newSymptoms = values.symptoms.filter((_, i) => i !== idx);
                                                                    ...
                                                                    <textarea
                                                                        ...
                                                                        onKeyDown={(e) => {
                                                                            if (e.key === "Enter") {
                                                                                ...
                                                                                if (trimmed && !values.symptoms.includes(trimmed)) {
                                                                                    setFieldValue("symptoms", [...values.symptoms, trimmed]);
                                                                                    setNewSymptom("");
                                                                                }
                                                                            }
                                                                        }}
