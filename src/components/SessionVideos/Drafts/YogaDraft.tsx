...
const YogaDraft: React.FC<YogaProps> = ({ videos, index, goBack, onEdit, onDelete }) => {
                                ...
                                <div key={video._id} className="border border-[#B1A9A9] rounded-lg">
                                    <div className="relative">
                                        {video.thumbnailUrl ? (
                                            ...
                                            <video src={video.videoUrl.startsWith('http') ? video.videoUrl : `${apiUrl}${video.videoUrl}`} className="w-full h-[169px] rounded-tl-lg rounded-tr-lg overflow-hidden block object-cover" preload="metadata" />
                                        )}
