...
interface UploadProps {
    ...
    searchTerm: string;
    onSelectCard: (index: number, data: FormDataType) => void;
}

const Upload: React.FC<UploadProps> = ({ formData, onEdit, onDelete, searchTerm, onSelectCard }) => {
        ...
        <div className="px-[24px] mt-[40px]">
            {filteredData.length === 0 ? (
                ...
                <div className="grid grid-cols-4 gap-4">
                    {filteredData.map((form, cardIndex) => (
                        <div key={cardIndex} className="border border-[#B1A9A9] rounded-lg cursor-pointer" onClick={() => onSelectCard(cardIndex, form)}>
                            <ul>
                                {form.videos.length > 0 && (
                                    <div className="relative">
                                        <video
                                            controls
                                            src={URL.createObjectURL(form.videos[0])}
                            ...
                            <div className="px-[8px] py-[8px] space-y-2">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-[#11401C] font-semibold">{form.title}</h3>
                                ...
                                <p className="text-[#4D4D4D] font-normal text-[12px]">{form.description}</p>
