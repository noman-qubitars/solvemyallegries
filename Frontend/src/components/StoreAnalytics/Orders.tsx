...
import { StoreOrdersData as initialData } from "@/data/StoreAnalysis";
...
const Orders: React.FC<OrdersProps> = ({ searchTerm, selectedFilter }) => {
    ...
    const statusStyles: Record<string, string> = {
        Delivered: "bg-[#E9F8EC] text-[#21BA45]", 
        Processing: "bg-[#FEF1E8] text-[#F2711C]",  
        Shipped: "bg-[#E9F3FA] text-[#2185D0]",   
        Cancelled: "bg-[#FBE9E9] text-[#DB2828]", 
    };
                            ...
                            currentItems.map((user, index) => (
                                <tr key={index} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-4 flex justify-center whitespace-nowrap">
                                        <div
                                            className={`flex items-center gap-2 px-[22px] w-fit py-[10px] rounded-[12px] text-[14px] font-medium ${ statusStyles[user.balance] ?? "bg-gray-100 text-gray-500"}`}
                                        >
